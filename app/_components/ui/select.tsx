...
>(({ className, children, position = "popper", sideOffset = 8, ...props }, ref) => (
  <SelectPrimitive.Portal>
    <SelectPrimitive.Content
      ...
      className={cn(
        // Caixa visível e sólida
        "z-[9999] overflow-hidden rounded-xl border border-zinc-200 bg-white text-zinc-900 shadow-lg",
        // Largura coerente com o trigger
        "min-w-[var(--radix-select-trigger-width)]",
        // Evita estourar e ficar “colado” em conteúdo
        "max-h-[var(--radix-select-content-available-height)]",
        // Animações e transições no padrão shadcn
        "data-[state=open]:animate-in data-[state=closed]:animate-out",
        "data-[state=closed]:fade-out-0 data-[state=open]:fade-in-0",
        "data-[state=closed]:zoom-out-95 data-[state=open]:zoom-in-95",
        "data-[side=bottom]:slide-in-from-top-2 data-[side=top]:slide-in-from-bottom-2",
        // Garante interação mesmo se existir estilo global agressivo
...
>(({ className, ...props }, ref) => (
  <SelectPrimitive.Label
    ref={ref}
    className={cn("px-2 py-1.5 text-sm font-medium text-zinc-700", className)}
    {...props}
  />
));
